...
def happy_couple(H, k):
	'Method to calculate k happy couples'
...
def test():
	...
	for g in new:
		...
		for l in boy_lists:
			log_maker(k.name + '  is searching ' + l.name)
			if l.name != g.boy.name and l.relationship_status == 'single' and k.relationship_status == 'single' and (k.is_eligible(l.boys_girlfriend_budget)) and (l.is_eligible(k.maintenance_budget, k.attractiveness)) :
				k.relationship_status = 'in_a_relationship'
				l.relationship_status = 'in_a_relationship'
				k.boyfriend = l.name
				l.girlfriend = k.name
